...
| D4 | **The 2,738 simulated-business lead records** that still hold fake names and emails. | Strip them (it's test data). |
| D5 | **Sync on connect**: run the first data pull right after a client connects, instead of waiting for 2 AM UTC. | Yes. New clients see numbers the same day. |
| D6 | **AI links section** (ChatGPT/Claude/Perplexity/Grok) on the homepage. | Leave it off. No proven SEO effect, and it sends visitors away. |
| D7 | **Intake form's job** now that self-serve is live. | Free-audit request now; "Partner with us / talk to Nick" once the free audit is in Stripe. |
| D8 | **Apex status**: 0 ad-spend rows, no active campaigns, last directive Aug 13. | Tell Claude: paused, churned, or should it investigate a sync problem? |
